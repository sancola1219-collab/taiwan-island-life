...
const BUILDING_DRAWS={
 seven(b){const x=b.x,y=b.y,w=b.w,h=b.h;bShadow(x,y+h,w); // v52 7-ELEVEN：白牆＋橘綠紅三色帶
  ctx.fillStyle='#f4f4f0';rr(x,y-30,w,h+30,8);ctx.fill();
  ctx.fillStyle='#0aa15a';ctx.fillRect(x,y-30,w,7);
  ctx.fillStyle='#f28a1e';ctx.fillRect(x,y-23,w,7);
  ctx.fillStyle='#e2453c';ctx.fillRect(x,y-16,w,7);
  ctx.fillStyle='#8fd0e8';rr(x+6,y+2,w-12,h-12,4);ctx.fill();
  ctx.fillStyle='#5a5a5a';rr(x+w/2-10,y+h-28,20,28,3);ctx.fill();
  drawRoofSign(x+w/2,y-46,'7-ELEVEN','#0a7a45');},
 familym(b){const x=b.x,y=b.y,w=b.w,h=b.h;bShadow(x,y+h,w); // v52 全家：白牆＋藍綠雙色帶
  ctx.fillStyle='#f6f8fa';rr(x,y-30,w,h+30,8);ctx.fill();
  ctx.fillStyle='#00a0e9';ctx.fillRect(x,y-30,w,10);
  ctx.fillStyle='#7cc623';ctx.fillRect(x,y-20,w,10);
  ctx.fillStyle='#bfe3f2';rr(x+6,y+2,w-12,h-12,4);ctx.fill();
  ctx.fillStyle='#5a6a72';rr(x+w/2-10,y+h-28,20,28,3);ctx.fill();
  drawRoofSign(x+w/2,y-46,'全家便利商店','#0068b7');},
 mcd(b){const x=b.x,y=b.y,w=b.w,h=b.h;bShadow(x,y+h,w); // v52 麥當勞：米牆紅簷＋金拱門M
  ctx.fillStyle='#f8f0dc';rr(x,y-28,w,h+28,8);ctx.fill();
  ctx.fillStyle='#da2f28';rr(x,y-28,w,12,6);ctx.fill();
  ctx.fillStyle='#8fd0e8';rr(x+6,y+4,w-12,h-14,4);ctx.fill();
  ctx.strokeStyle='#ffc400';ctx.lineWidth=7;ctx.lineCap='round'; // 金拱門
  ctx.beginPath();ctx.arc(x+w/2-11,y-4,13,Math.PI,0);ctx.stroke();
  ctx.beginPath();ctx.arc(x+w/2+11,y-4,13,Math.PI,0);ctx.stroke();ctx.lineCap='butt';
  drawRoofSign(x+w/2,y-44,'麥當勞','#da2f28');},
 kfc(b){const x=b.x,y=b.y,w=b.w,h=b.h;bShadow(x,y+h,w); // v52 肯德基：白牆＋紅白條紋簷
  ctx.fillStyle='#faf6f0';rr(x,y-30,w,h+30,8);ctx.fill();
  for(let i=0;i<6;i++){ctx.fillStyle=i%2?'#a6192e':'#fff';
    ctx.beginPath();ctx.moveTo(x+i*w/6,y-30);ctx.lineTo(x+(i+1)*w/6,y-30);
    ctx.lineTo(x+(i+1)*w/6,y-12);ctx.arc(x+(i+0.5)*w/6,y-12,w/12,0,Math.PI);ctx.lineTo(x+i*w/6,y-12);ctx.fill();}
  ctx.fillStyle='#8fd0e8';rr(x+6,y+4,w-12,h-14,4);ctx.fill();
  ctx.fillStyle='#a6192e';rr(x+w/2-10,y+h-28,20,28,3);ctx.fill();
  drawRoofSign(x+w/2,y-46,'肯德基','#a6192e');},
 rentcar(b){const x=b.x,y=b.y,w=b.w,h=b.h;bShadow(x,y+h,w); // v52 租車行：車庫＋展示小紅車
  ctx.fillStyle='#dfe4e8';rr(x,y-32,w,h+32,8);ctx.fill();
  ctx.fillStyle='#4a6f8f';ctx.fillRect(x,y-32,w,9);
  ctx.fillStyle='#3a3f44';rr(x+8,y-14,w-16,h+8,5);ctx.fill(); // 車庫口
  let g=ctx.createLinearGradient(x,y+h-26,x,y+h-6); // 展示車
  g.addColorStop(0,'#f26a5a');g.addColorStop(1,'#c93a2e');
  ctx.fillStyle=g;rr(x+w/2-20,y+h-22,40,13,6);ctx.fill();
  ctx.fillStyle='#bfe3f2';rr(x+w/2-12,y+h-20,22,6,3);ctx.fill();
  ctx.fillStyle='#2a2a2a';ctx.beginPath();ctx.arc(x+w/2-11,y+h-8,5,0,7);ctx.arc(x+w/2+11,y+h-8,5,0,7);ctx.fill();
  drawRoofSign(x+w/2,y-48,'🚗 租車行','#4a6f8f');},
...
const SIZE={t101:[4,3],shop:[5,3],market:[8,2],teahouse:[4,3],queenhead:[2,2],lantern:[3,2],
  seven:[3,2],familym:[3,2],mcd:[3,2],kfc:[3,2],rentcar:[4,3], // v52 連鎖店＋租車行
...
/* v52 連鎖店/租車行放置：環形搜索＋空間網格快速重疊查詢 */
function anyBuildOverlap(hx,hy,w,h){ // 用 BGRID 快查（含1格邊距）
  const x0=(hx-1)*TILE, y0=(hy-1)*TILE, x1=(hx+w+1)*TILE, y1=(hy+h+2)*TILE;
  const seen=new Set();
  for(let gy=Math.floor(y0/GCELL);gy<=Math.floor(y1/GCELL);gy++)
    for(let gx=Math.floor(x0/GCELL);gx<=Math.floor(x1/GCELL);gx++){
      const a=BGRID.get(gx*1000+gy); if(!a)continue;
      for(const b of a){ if(seen.has(b))continue; seen.add(b);
        if(hx<b.tx+b.tw+1&&hx+w+1>b.tx&&hy<b.ty+b.th+2&&hy+h+1>b.ty)return true; } }
  return false; }
function findStoreSpot(cx,cy,w,h,r0){ // 由內而外沿環找可蓋店的空地
  for(let r=(r0||2);r<=16;r++){
    const n=r*8;
    for(let k=0;k<n;k++){ const ang=k/n*6.283;
      const hx=Math.round(cx+Math.cos(ang)*r), hy=Math.round(cy+Math.sin(ang)*r*0.8);
      let ok=true;
      for(let dy=-1;dy<=h&&ok;dy++)for(let dx=-1;dx<=w&&ok;dx++){const t=T(hx+dx,hy+dy);
        if(!(t===GRASS||t===FIELD||t===PATH||t===PLAZA||t===SAND||t===HIGH))ok=false;} // 含沙地(離島)與高地(拉薩/烏魯木齊)
      if(ok&&!anyBuildOverlap(hx,hy,w,h))return [hx,hy];
    }}
  return null; }
/* v52 四大連鎖店：每個縣市/省份的首要城鎮各放一組 7-11/全家/麥當勞/肯德基 */
function genBrandStores(townArr){
  const brands=[
    ['seven','7-ELEVEN',{}],
    ['familym','全家便利商店',{}],
    ['mcd','麥當勞',{food:'大麥克套餐',price:150,icon:'🍔'}],
    ['kfc','肯德基',{food:'香脆炸雞桶',price:200,icon:'🍗'}]];
  const seen=new Set(), seats=[];
  for(const t of townArr){ if(seen.has(t.c))continue; seen.add(t.c); seats.push(t); }
...
function genExtraShops(townArr,keys,regName){
  for(const k of keys){ const tw=townArr.find(t=>(t.c&&t.c.includes(k))||t.n.includes(k)); if(!tw)continue;
    for(const [tp,lb] of [['petshop','寵物店・'+tw.n],['giftshop','禮品店・'+tw.n],['registry',regName+'・'+tw.n]]){
      const [w,h]=SIZE[tp]||[3,3];
...
function genMapJP(m){ // v57 日本群島：北海道/本州/四國/九州/沖繩＋津輕/瀨戶內海/關門海峽
  const N=v=>(vnoise(v*7.3+2.5,v*3.1+8.2)-0.5);
  const HON=[[390,200],[376,264],[364,332],[354,386],[316,412],[280,434],[242,460],[204,484],[166,508],[136,528]]; // 本州弧線
  const HOK=[[352,160],[382,118],[408,86],[444,94],[434,138],[398,164]]; // 北海道
  for(let ty=0;ty<MH;ty++)for(let tx=0;tx<MW;tx++){
    let land=false;
    if(polylineDist(tx,ty,HON)<46+N(tx*0.9+ty*0.7)*10)land=true;               // 本州帶
    if(Math.hypot(tx-356,ty-388)<50+N(tx+ty)*8)land=true;                       // 關東平原
    if(Math.hypot(tx-280,ty-370)<40+N(tx-ty)*8)land=true;                       // 北陸/能登
    if(polylineDist(tx,ty,HOK)<40+N(tx*1.1+ty)*9)land=true;                     // 北海道
    if(((tx-200)/48)**2+((ty-548)/26)**2<1)land=true;                           // 四國
    if(((tx-92)/52)**2+((ty-598)/62)**2<1)land=true;                            // 九州
    if(((tx-158)/20)**2+((ty-733)/11)**2<1)land=true;                           // 沖繩
    // 海峽（強制海面）：津輕、瀨戶內海、關門
    if(((tx-374)/24)**2+((ty-182)/13)**2<1)land=false;
    if(((tx-192)/55)**2+((ty-521)/7)**2<1)land=false;
    if(((tx-122)/10)**2+((ty-542)/7)**2<1)land=false;
    let t=SEA;
    if(land){ t=GRASS;
      if(polylineDist(tx,ty,HON)<12&&vnoise(tx*0.07,ty*0.07)>0.48)t=HIGH;       // 本州中央山地
      if(polylineDist(tx,ty,HON)<7&&vnoise(tx*0.07+40,ty*0.07)>0.62)t=MT;
      if(polylineDist(tx,ty,HOK)<12&&vnoise(tx*0.08+9,ty*0.08)>0.55)t=HIGH;     // 北海道山地
      if(((tx-268)/6)**2+((ty-430)/10)**2<1)t=LAKE;                             // 琵琶湖
      if(t===GRASS&&vnoise(tx*0.05+80,ty*0.05)>0.74)t=FIELD; }
    m[ty*MW+tx]=t; }
  const sand=[];
  for(let ty=0;ty<MH;ty++)for(let tx=0;tx<MW;tx++){ const t=m[ty*MW+tx];
...
function genWorldJP(){ // v57 於 map=jpMap、BUILDINGS=jpB 的狀態下呼叫（比照 genWorldCN）
  BGRID=new Map(); // 清掉前一世界網格，避免建置期重疊誤判
  if(typeof JP_STATIONS!=='undefined')JP_STATIONS.forEach(s=>{const [sx,sy]=fitSpot(s.tx,s.ty,5,3);addBuild('station',sx,sy,5,3,s.n);});
  AIRPORTS.filter(a=>a.w==='jp').forEach(a=>{const [ax,ay]=fitSpot(a.tx,a.ty,5,3);addBuild('airport',ax,ay,5,3,a.label,{airport:a});});
  LANDMARKS_JP.forEach(L=>{const [tw,th]=SIZE[L.t];const [lx,ly]=fitSpot(L.tx,L.ty,tw,th);
    addBuild(L.t,lx,ly,tw,th,L.label,{lines:L.lines,county:L.county,isLm:true});});
  EATERIES_JP.forEach(e=>{const [ex,ey]=fitSpot(e.tx,e.ty,3,2);addBuild('eatery',ex,ey,3,2,e.label,{food:e.food,price:e.price,icon:e.icon});});
  HOTELS_JP.forEach(h=>{const [hx,hy]=fitSpot(h.tx,h.ty,4,3);addBuild('hotel',hx,hy,4,3,h.label);});
  { rs=SEED+177; const SUR=['佐藤','鈴木','高橋','田中','伊藤','渡邊','山本','中村','小林','加藤','吉田','山田','松本','井上','木村','齋藤'];
    const WALLS=['#ecd6b2','#e6dcc4','#f0e8dc','#dfe4d0','#e8e0d0']; const ROOFS=['#5a6a7a','#8a6a4a','#6f8a9a','#4a5a6a','#7a6a5a'];
    const offs=[[-7,-3],[6,-3],[-5,4],[6,4],[-8,1],[8,1],[-3,6],[4,-6],[-6,-6],[7,-1]];
    for(const tw of TOWNS_JP){ let placed=0;
      for(const [ox,oy] of offs){ if(placed>=3)break;
        ...
        if(ok){const nm=SUR[Math.floor(rand()*SUR.length)];
          addBuild('house',hx,hy,2,1,'🏠 '+nm+'家',{wall:WALLS[Math.floor(rand()*WALLS.length)],roof:ROOFS[Math.floor(rand()*ROOFS.length)]});placed++;}
      }}}
  genBrandStores(TOWNS_JP); // 四大連鎖：每都道府縣一組
  genRentcars(TOWNS_JP);    // 每都道府縣一間租車行
  genExtraShops(TOWNS_JP,['名古屋','福岡','札幌','廣島','仙台','金澤'],'區役所');
}
function genWorldCN(){ // 於 map=cnMap、BUILDINGS=cnB 的狀態下呼叫
  BGRID=new Map(); // v52 清掉台灣網格，避免大陸建置時的重疊查詢誤判（開機尾端會再 rebuildGrids）
...
{ // v57 建立日本世界（作法同大陸）
  const jpMap=new Uint8Array(MW*MH); genMapJP(jpMap);
  const _map=map,_b=BUILDINGS; map=jpMap; BUILDINGS=[]; world='jp';
  genWorldJP();
  for(const b of BUILDINGS)if(!SCALE_EXEMPT.has(b.t))b.scale=BSCALE;
  SCENES.jp={map:jpMap,buildings:BUILDINGS,mini:makeMini(jpMap),towns:TOWNS_JP,name:'日本'};
  map=_map; BUILDINGS=_b; world='tw';
}
function worldName(w){return w==='cn'?'大陸':w==='jp'?'日本':'台灣';} // v57
function setScene(w){ const S=SCENES[w]; if(!S)return; world=w; map=S.map; BUILDINGS=S.buildings; mini=S.mini;
  ACT_TOWNS=S.towns; ACT_NPCDEFS=(w==='cn')?CN_NPC_DEFS:(w==='jp')?JP_NPC_DEFS:NPC_DEFS; // v57 日本NPC
...
function rescueTeleport(){ // v44 保證真的把玩家移到「另一座」最近城市，且每次落點不同
  player.riding=null;player.balloonRide=null;player.ferris=null;player.soak=null;player.pray=null;player.fishing=null;player.sailing=false;
  for(const pt of player.pets||[])pt.atk=null;
  zoomT=1; flyAnim=0;
...
function drawFlyAnim(){ // v42 飛機飛出去/降落動畫（螢幕層）
  ...
  ctx.fillText('✈️ 飛往「'+flyLabel+'」（'+worldName(flyDestW)+'）',VW/2,58);ctx.textAlign='left';
}
...
function fruitOf(tx,ty){
  if(world==='jp'){ // v57 日本：北水蜜桃(青森岡山)、中橘子(蜜柑)、南芒果(沖繩)
    if(ty<MH*0.35)return '水蜜桃';
    if(ty<MH*0.75)return '橘子';
    return '芒果'; }
...
function buildRailNet(){
  const nodes={}, edges=[];
  const NETLINES=world==='cn'?(typeof CN_RAIL_LINES!=='undefined'&&CN_RAIL_LINES):world==='jp'?(typeof JP_RAIL_LINES!=='undefined'&&JP_RAIL_LINES):null; // v57 泛化：大陸/日本共用路網建構
  const NETSTS=world==='cn'?CN_STATIONS:world==='jp'?JP_STATIONS:null;
  if(NETLINES&&NETSTS){
    NETSTS.forEach(s=>{nodes[s.city]={x:s.tx,y:s.ty,st:s.n};});
    NETLINES.forEach(L=>{
      let prev=null, bends=[];
      L.s.forEach(el=>{
        ...
        if(prev&&nodes[prev]){
          ...
          edges.push({a:prev,b:el,line:L.n,t:L.t,ferry:!!L.ferry,bridge:!!L.bridge,tunnel:!!L.tunnel,note:L.note,pts,len}); // v57 橋/隧道旗標
        }
        prev=el; bends=[];
      });
    });
...
function railRoute(from,to){ // 最短路徑；回傳{pts(px),len(格),lines(依序經過線名),reg,ferry}
  ...
  const pts=[],lines=[]; let reg=false,ferry=false,tunnel=false;
  segs.forEach(s=>{
    ...
    if(s.e.t==='reg')reg=true; if(s.e.ferry)ferry=true; if(s.e.tunnel)tunnel=true;});
  return {pts:pts.map(([x,y])=>({x:x*TILE,y:y*TILE})),len:D[to],lines,reg,ferry,tunnel};
}
function startRide(pts,kind,speed,onEnd){
  ...
  player.fishing=null; player.sailing=false; player.driving=false; ui=null; menu=null;
}
...
function boarding(){return player.riding||player.balloonRide||player.ferris||player.sailing||player.driving;} // v52 開車也算同乘：夥伴/寵物貼齊隱藏、下車自動出現
...
function buildAct(b){
  ...
  const L={t101(){ openMenu('台北101：全台最高樓！',[
   ...
   station(){ const here=ACT_STATIONS.find(s=>s.n===b.label); if(!here)return;
     ...
     const rideTo=(s)=>{ const key=s.city||s.n, route=railRoute(myKey,key);
       ...
       toast((route.ferry?'⛴️ 火車開上渡輪過海！':route.tunnel?'🚇 潛入海底隧道！':route.reg?'🚂 '+(world==='jp'?'在來線列車出發！':'綠皮火車哐當哐當出發！'):world==='cn'?'🚄 動車出發！':world==='jp'?'🚅 新幹線出發！':'🚂 火車出發！')+'經由：'+via);};
     ...
     if(world!=='tw'){ // v45 站多：先選地區再選城市；票價依實際路徑長度（v57 日本沿用）
       const regions={};
   ...
   harbor(){ const opts=[];
     ...
     if(player.boat){ // v55 五船型×五色
       opts.push({label:'🚢 換船型（現：'+(BOAT_TYPES.find(t=>t[0]===(player.boatType||'fishing'))||BOAT_TYPES[0])[1]+'）',cb(){
         openMenu('🚢 選擇船型（免費更換）',[...BOAT_TYPES.map(([id,nm])=>({
           label:((player.boatType||'fishing')===id?'✅ ':'')+nm,cb(){
             player.boatType=id;sfx('chime');save();toast('🚢 換成「'+nm+'」！出海看看吧');buildAct(b);}})),
           {label:'回上頁',cb(){buildAct(b);}}]);}});
       opts.push({label:'🎨 換船色（現：'+(BOAT_COLORS.find(c=>c[0]===(player.boatColor||'#c98a4a'))||BOAT_COLORS[0])[1]+'）',cb(){
         openMenu('🎨 選擇船色（免費更換）',[...BOAT_COLORS.map(([cc,nm])=>({
           label:((player.boatColor||'#c98a4a')===cc?'✅ ':'')+nm,cb(){
             player.boatColor=cc;sfx('chime');save();toast('🎨 船身換成「'+nm+'」！');buildAct(b);}})),
           {label:'回上頁',cb(){buildAct(b);}}]);}}); }
   ...
   airport(){ // v40 機場：搭飛機往返台灣/大陸/日本（v57 三地互飛）
     const dests=AIRPORTS.filter(a=>a.w!==world);
     const opts=dests.map(a=>({label:'✈️ 飛往「'+a.label+'」（'+worldName(a.w)+'）機票 '+fmt(1500)+'元',cb(){
       if(money<1500){dlg(b.label,['機票一張 '+fmt(1500)+' 元喔。','努力賺錢再來搭飛機吧！']);return;}
       money-=1500; flyTo(a.w,a.label); }}));
     opts.push({label:'看看航班資訊',cb(){dlg(b.label,['歡迎搭乘！本機場提供台灣⇌大陸⇌日本航線。',
       world==='tw'?'飛去大陸看長城熊貓找劉思思，或飛日本看富士山吃壽司～':
       world==='cn'?'飛回台灣環島，或飛日本泡溫泉搭新幹線～':'飛台灣環島或大陸看長城，旅途愉快～','（機票每張 '+fmt(1500)+' 元）']);}});
     opts.push({label:'離開',cb(){ui=null;}});
     openMenu('✈️ '+b.label,opts); },
   petshop(){ // v41 寵物店：領養貓狗、買飼料、v49 可養2隻＋寵物飾品
     const full=(player.pets||[]).length>=PET_MAX;
     openMenu('🐾 '+b.label+'（已養 '+(player.pets||[]).length+'/'+PET_MAX+' 隻）',[
       {label:'🐕 領養狗狗（'+fmt(2000)+'元）忠心護主',cb(){ if(full){dlg(b.label,['你已經養了 '+PET_MAX+' 隻寵物囉！','最多只能養 '+PET_MAX+' 隻，先照顧好牠們吧～']);return;}
         if(money<2000){dlg(b.label,['領養一隻要 '+fmt(2000)+' 元喔。']);return;} money-=2000; adoptPet('dog'); ui=null;}},
       {label:'🐈 領養貓咪（'+fmt(2000)+'元）優雅高冷',cb(){ if(full){dlg(b.label,['你已經養了 '+PET_MAX+' 隻寵物囉！','最多只能養 '+PET_MAX+' 隻，先照顧好牠們吧～']);return;}
       ...
       {label:'🎀 寵物飾品店（幫毛孩打扮）',cb(){ petAccShop(b); }},
       {label:'看看介紹',cb(){dlg(b.label,['歡迎光臨寵物店！','領養的貓狗會跟著你、有好感度，好感高會冒愛心和音符🎵。','最多可以養 '+PET_MAX+' 隻，也可以在路上撿流浪貓狗免費領養喔！','寵物會慢慢長大變老…好好照顧牠、按時餵飼料吧！','🎀 還能買飾品幫牠們打扮！面對寵物可指使牠攻擊路人（後果自負）。']);}},
       {label:'離開',cb(){ui=null;}}]); },
  ...
  L.seven=L.shop; L.familym=L.shop; L.mcd=L.eatery; L.kfc=L.eatery; // v52 連鎖店沿用雜貨/小吃行為
  L.rentcar=function(){ const opts=[]; // v52 租車
    if(!player.carRent) opts.push({label:'🚗 租一台小紅車（3,000 元・永久持有）',cb(){
      if(money<3000){dlg(b.label,['租車一台 3,000 元喔！','租了以後隨時按互動鍵就能上下車，','速度比警車還快一點～']);return;}
      money-=3000;player.carRent=true;player.driving=true;ui=null;sfx('cash');save();
      toast('🚗 租到車了！比警車還快～（按互動鍵可下車）');}});
    else{ opts.push({label:player.driving?'🅿️ 下車步行':'🚗 上車出發！',cb(){
        player.driving=!player.driving;ui=null;sfx('pop');save();
        toast(player.driving?'🚗 上車！出發～':'🅿️ 已下車');}});
      opts.push({label:'🚙 換車型（現：'+(CAR_TYPES.find(t=>t[0]===(player.carType||'sedan'))||CAR_TYPES[0])[1]+'）',cb(){ // v55 五車型
        openMenu('🚙 選擇車型（免費更換）',[...CAR_TYPES.map(([id,nm])=>({
          label:((player.carType||'sedan')===id?'✅ ':'')+nm,cb(){
            player.carType=id;sfx('chime');save();toast('🚙 換成「'+nm+'」！');buildAct(b);}})),
          {label:'回上頁',cb(){buildAct(b);}}]);}});
      opts.push({label:'🎨 換車色（現：'+(CAR_COLORS.find(c=>c[0]===(player.carColor||'#c93a2e'))||CAR_COLORS[0])[1]+'）',cb(){ // v55 五車色
        openMenu('🎨 選擇車色（免費更換）',[...CAR_COLORS.map(([cc,nm])=>({
          label:((player.carColor||'#c93a2e')===cc?'✅ ':'')+nm,cb(){
            player.carColor=cc;sfx('chime');save();toast('🎨 烤漆換成「'+nm+'」！');buildAct(b);}})),
          {label:'回上頁',cb(){buildAct(b);}}]);}});
      opts.push({label:'↩️ 退租（退回 1,500 元）',cb(){
        player.carRent=false;player.driving=false;money+=1500;ui=null;sfx('cash');save();
        toast('↩️ 車子還給租車行，退回 1,500 元。');}}); }
    opts.push({label:'離開',cb(){ui=null;}});
    openMenu('🚗 '+b.label+'：想開車環遊嗎？',opts); };
...
// v49 玩家正對著門口的商店/設施？——用來讓「進店」優先於門口亂晃的貓狗/路人/樹木
function buildingInFront(){
  const p=frontPoint(38); let best=null,bd=1e9;
  for(const b of BUILDINGS){ if(b.t==='house'||b.t==='myhome')continue;
    const s=b.scale||1; let x0,y0,x1,y1;
    if(s===1){x0=b.x;y0=b.y;x1=b.x+b.w;y1=b.y+b.h;}
    else{const cx=b.x+b.w/2,by=b.y+b.h,hw=b.w*s/2,hh=b.h*s;x0=cx-hw;y0=by-hh;x1=cx+hw;y1=by;}
    if(p.x>x0-6&&p.x<x1+6&&p.y>y0-6&&p.y<y1+14){ // 正前方 38px 落在建物範圍內＝面對門口
      const d=dist(player.x,player.y,(x0+x1)/2,y1); if(d<bd){bd=d;best=b;} } }
  return best;
}
...
function regionOf(){ const tx=player.x/TILE, ty=player.y/TILE;
  ...
  const nt=nearestTown(tx,ty); if(!nt.t)return worldName(world);
...
function arrest(){
  ...
  player.soak=null; player.pray=null; player.sailing=false; player.driving=false; zoomT=1;
  const pr=nearestPrison();
...
function interact(){
  ...
  if(player.tool===6&&player.toy==='次元砲'&&godz&&dist(godz.x,godz.y,player.x,player.y)<1900){fireCannon();return;} // v55 陸地/船上都能迎戰哥吉拉（射程加大）
  ...
  if(!player.sailing){
    ...
    { const fb=buildingInFront(); if(fb){buildAct(fb);return;} } // v49 面對店門/設施＝優先進店，避免門口的貓狗/路人/樹木攔截互動
    for(const pt of (player.pets||[])){ if(dist(pt.x,pt.y,p.x,p.y)<52){petInteract(pt);return;} } // v41/v49 面對寵物→指使/照顧（可多隻）
    for(let i=strays.length-1;i>=0;i--){const s=strays[i]; // v41 面對流浪貓狗→免費領養
      if(dist(s.x,s.y,p.x,p.y)<52||dist(s.x,s.y,player.x,player.y)<48){
        if((player.pets||[]).length>=PET_MAX){dlg('流浪'+(s.kind==='dog'?'狗':'貓'),['一隻流浪'+(s.kind==='dog'?'狗':'貓')+'…','你已經養滿 '+PET_MAX+' 隻寵物了，先照顧好牠們吧！']);return;}
  ...
  if(player.boat){ const ft=frontTile(1.3);
    if(ft.t===SEA||ft.t===LAKE){player.x=ft.x;player.y=ft.y;player.sailing=true;
      player.driving=false; sfx('splash');toast('出海囉！⛵');return;} }
  // v52 上/下車（已租車、且沒有其他互動目標時）
  if(player.carRent&&!player.sailing){ player.driving=!player.driving; sfx('pop');
    toast(player.driving?'🚗 上車！出發～（比警車還快）':'🅿️ 已下車'); save(); return; }
}
...
const PET_MAX=4; // v52 上限提高：最多養四隻
// v49 寵物飾品（戴在頭上）
const PET_ACCS=[
 {id:'collar',n:'鈴鐺項圈',e:'🔔',price:600},
 {id:'ribbon',n:'蝴蝶結',e:'🎀',price:600},
 {id:'crown',n:'小皇冠',e:'👑',price:2500},
 {id:'hat',n:'紳士帽',e:'🎩',price:1500},
 {id:'flower',n:'花圈',e:'🌸',price:800},
 {id:'scarf',n:'圍巾',e:'🧣',price:1000},
];
function petAgeDaysOf(p){ return p?((absMin()-p.born)/1440):0; }
function petStageOf(p){ const d=petAgeDaysOf(p); return d<3?'幼':d<16?'成':'老'; }
function makePetObj(kind,name){ return {kind,name:name||PET_NAMES[Math.floor(Math.random()*PET_NAMES.length)],
  born:absMin()-1440, aff:20,hunger:90, x:player.x,y:player.y+30, face:0,walk:0,vx:0,vy:0, sick:0, atk:null, note:0, acc:null, slot:(player.pets||[]).length}; }
function askPetName(kind,cur){ // v52 幫寵物取名字（取消/留空則用預設）
  try{ const nm=(prompt('幫'+(kind==='dog'?'狗狗':'貓咪')+'取個名字吧！(最多6個字)',cur||'')||'').trim();
    return nm?nm.slice(0,6):null; }catch(e){ return null; } }
function adoptPet(kind,name){ player.pets=player.pets||[];
  if(player.pets.length>=PET_MAX){toast('你已經養滿 '+PET_MAX+' 隻寵物了！先放生一隻才能再養。');return false;}
  const p=makePetObj(kind,name);
  const nm=askPetName(kind,p.name); if(nm)p.name=nm; // v52 領養時取名
  p.x=player.x; p.y=player.y+30; player.pets.push(p); reflowPetSlots(); sfx('jingle'); save();
  toast('🐾 你領養了'+(kind==='dog'?'狗狗':'貓咪')+'「'+p.name+'」！面對牠按互動鍵可以摸摸/餵食/取名/指使攻擊。'); return true; }
function reflowPetSlots(){ (player.pets||[]).forEach((p,i)=>p.slot=i); } // 跟隨隊形站位
function petAccShop(b){ // v49 寵物飾品店
  const pets=player.pets||[];
  if(!pets.length){dlg(b.label,['你還沒有寵物耶…','先領養一隻貓貓狗狗，再來幫牠打扮吧！']);return;}
  const pick=pets.length===1?pets[0]:null;
  if(!pick){ openMenu('🎀 要幫哪隻毛孩打扮？',[
    ...pets.map(p=>({label:(p.kind==='dog'?'🐕':'🐈')+' '+p.name+(p.acc?'（現戴'+(PET_ACCS.find(a=>a.id===p.acc)||{}).e+'）':''),cb(){petAccMenu(b,p);}})),
    {label:'返回',cb(){buildAct(b);}}]); return; }
  petAccMenu(b,pick);
}
function petAccMenu(b,p){
  player.ownPetAccs=player.ownPetAccs||[]; // v49 買過的寵物飾品永久擁有（兩隻共用衣櫃）
  const opts=PET_ACCS.map(a=>{ const owned=player.ownPetAccs.includes(a.id);
    return {label:(p.acc===a.id?'✅ ':'')+a.e+a.n+(owned?'（已擁有・點穿/脫）':'（'+fmt(a.price)+'元）'),cb(){
    if(p.acc===a.id){p.acc=null;save();toast('取下了'+p.name+'的'+a.n+'。');petAccMenu(b,p);return;}
    if(!owned){ if(money<a.price){dlg(b.label,[a.n+' 要 '+fmt(a.price)+' 元喔。']);return;}
      money-=a.price; player.ownPetAccs.push(a.id); sfx('cash'); }
    p.acc=a.id; save();
    toast('🎀 幫'+p.name+'戴上「'+a.n+'」'+a.e+'，好可愛！'); petAccMenu(b,p);}};});
  if(p.acc)opts.push({label:'🚫 拿掉飾品',cb(){p.acc=null;save();toast('取下了'+p.name+'的飾品。');petAccMenu(b,p);}});
  opts.push({label:'返回',cb(){petAccShop(b);}});
  openMenu('🎀 '+p.name+' 的飾品（買過即擁有，兩隻可共用）',opts);
}
function petInteract(p){ if(!p)return;
  ...
  const opts=[
   ...
   {label:'✏️ 幫牠改名字',cb(){ const nm=askPetName(p.kind,p.name); // v52 隨時改名
     if(nm){const old=p.name;p.name=nm;sfx('chime');save();toast('✏️ 「'+old+'」改名為「'+nm+'」！');}
     ui=null;}},
   {label:'🏞️ 放生（送養）',cb(){openMenu('確定要放生「'+p.name+'」嗎？',[
     {label:'放生…',cb(){toast('你和'+p.name+'道別，牠回到了街上。');
       const i=player.pets.indexOf(p);if(i>=0)player.pets.splice(i,1);reflowPetSlots();sfx('sad');save();}},
     {label:'再想想',cb(){ui=null;}}]);}},
   {label:'離開',cb(){ui=null;}}];
  openMenu('🐾 '+p.name+' '+heart+(p.acc?(PET_ACCS.find(a=>a.id===p.acc)||{}).e:'')+'（'+petStageOf(p)+'齡・好感'+Math.floor(p.aff)+'/100・飽食'+Math.floor(p.hunger)+'）',opts);
}
function updatePet(dt){ const pets=player.pets||[]; if(!pets.length)return;
  for(let pi=pets.length-1;pi>=0;pi--){ const p=pets[pi];
    ...
    else{ p.aff=Math.max(0,p.aff-dt*0.18); } // 餓→好感降
    p.sick=0; // v52 寵物永遠健康：不再生病、不會死亡
    p.bark=(p.bark==null?4+Math.random()*8:p.bark)-dt; // v52 貓貓叫/汪汪叫
    if(p.bark<=0){ if(dist(p.x,p.y,player.x,player.y)<280&&!boarding()){
        p.say=1.6; p.cry=Math.floor(Math.random()*3); if(Math.random()<0.4)sfx('blip'); }
      p.bark=6+Math.random()*10; }
    if(p.say>0)p.say-=dt;
    if(p.atk){ const c=p.atk.c;                                    // 指使攻擊
      ...
      continue; }
    // v49 跟隨主人身後：每隻站不同橫向位置，避免疊在一起
    const off=(p.slot||0)===0?-14:16;
    const tx2=player.x-DIRV[player.face][0]*22+off, ty2=player.y+22, d=dist(p.x,p.y,tx2,ty2);
    if(d>900||boarding()){ p.x=tx2; p.y=ty2; continue; } // v49 落後太遠(換地圖/傳送/搭乘)＝直接瞬移回主人身邊，不會卡在別的地區
    if(d>38){ const a=Math.atan2(ty2-p.y,tx2-p.x), sp=Math.min(d,(d>220?340:170)*dt);
      const nx=p.x+Math.cos(a)*sp, ny=p.y+Math.sin(a)*sp;
      if(!hitObstacle(nx,ny)){p.x=nx;p.y=ny;}
      else if(!hitObstacle(nx,p.y))p.x=nx;
      else if(!hitObstacle(p.x,ny))p.y=ny;
      else { p.stuck=(p.stuck||0)+dt; if(p.stuck>1.2){p.x=tx2;p.y=ty2;p.stuck=0;} } // v49 卡住超過1.2秒＝瞬移解卡
      if(!hitObstacle(p.x,p.y))p.stuck=0;
      p.walk+=dt*10; p.face=Math.abs(tx2-p.x)>Math.abs(ty2-p.y)?(tx2>p.x?2:1):(ty2>p.y?0:3); } }
}
/* v55 車型/船型/顏色 */
const CAR_TYPES=[['sedan','小客車'],['sport','跑車'],['suv','休旅車'],['pickup','小貨車'],['taxi','計程車']];
const CAR_COLORS=[['#c93a2e','經典紅'],['#2e6bc9','海洋藍'],['#e8b520','亮黃'],['#3f8f5a','森林綠'],['#3a3a3a','曜石黑']];
const BOAT_TYPES=[['fishing','小漁船'],['yacht','遊艇'],['ocean','遠洋漁船'],['cruise','郵輪'],['carrier','航空母艦']];
const BOAT_COLORS=[['#c98a4a','木棕'],['#f5f5f0','純白'],['#c93a2e','緋紅'],['#2e6bc9','深藍'],['#7a8a99','艦灰']];
function drawRiderHead(x,y,hair,veil){ // v52 車窗探出的乘客頭
  ctx.fillStyle='#f5c99b';ctx.beginPath();ctx.arc(x,y,7,0,7);ctx.fill();
  ctx.fillStyle=hair||'#4a2f1d';ctx.beginPath();ctx.arc(x,y-1.5,7,Math.PI*1.02,Math.PI*1.98);ctx.fill();
  if(veil){ctx.fillStyle='rgba(255,255,255,.85)';ctx.beginPath();ctx.arc(x,y-4,6,Math.PI,0);ctx.fill();}
  ctx.fillStyle='#2e2620';ctx.beginPath();ctx.arc(x-2.3,y+1,1.1,0,7);ctx.arc(x+2.3,y+1,1.1,0,7);ctx.fill();}
function drawCarRide(){ // v52 租車：玩家＋伴侶一起上車，四方向（v55 五車型×五色）
  const x=player.x, y=player.y, f=player.face;
  const wed=player.love&&player.love.streamer&&player.love.stage==='married';
  const bump=player.moving?Math.sin(tGlobal*18)*0.8:0;
  const col=player.carColor||'#c93a2e', tp=player.carType||'sedan';
  const cabT=tp==='sport'?23:tp==='suv'?31:27; // 車頂高度：跑車低、休旅高
  ctx.fillStyle='rgba(0,0,0,.2)';
  if(f===1||f===2){ // 側面
    ctx.beginPath();ctx.ellipse(x,y+4,32,7,0,0,7);ctx.fill();
    ctx.save(); if(f===1){ctx.translate(x*2,0);ctx.scale(-1,1);}
    let g=ctx.createLinearGradient(x,y-cabT-3+bump,x,y+4+bump);
    g.addColorStop(0,tint(col,45));g.addColorStop(1,tint(col,-10));
    ctx.fillStyle=g;ctx.beginPath(); // 車身流線
    if(tp==='pickup'){ // 小貨車：前艙＋後斗
      ctx.moveTo(x-30,y+2+bump);ctx.lineTo(x-30,y-13+bump);ctx.lineTo(x-2,y-13+bump);
      ctx.lineTo(x-2,y-15+bump);ctx.lineTo(x+2,y-cabT+bump);ctx.lineTo(x+14,y-cabT+bump);ctx.lineTo(x+20,y-15+bump);
      ctx.quadraticCurveTo(x+30,y-13+bump,x+30,y-5+bump);ctx.lineTo(x+30,y+2+bump);ctx.closePath();ctx.fill();
    }else{
      ctx.moveTo(x-30,y+2+bump);ctx.lineTo(x-30,y-10+bump);ctx.quadraticCurveTo(x-29,y-15+bump,x-22,y-15+bump);
      ctx.lineTo(x-13,y-15+bump);ctx.lineTo(x-7,y-cabT+bump);ctx.lineTo(x+11,y-cabT+bump);ctx.lineTo(x+19,y-15+bump);
      ctx.quadraticCurveTo(x+30,y-13+bump,x+30,y-5+bump);ctx.lineTo(x+30,y+2+bump);ctx.closePath();ctx.fill();
    }
    ctx.strokeStyle='rgba(0,0,0,.22)';ctx.lineWidth=1.5;ctx.stroke();
    ctx.fillStyle='#bfe3f2';ctx.beginPath(); // 車窗
    if(tp==='pickup'){ctx.moveTo(x+4,y-cabT+2+bump);ctx.lineTo(x+13,y-cabT+2+bump);ctx.lineTo(x+17,y-16+bump);ctx.lineTo(x+4,y-16+bump);ctx.closePath();ctx.fill();}
    else{ctx.moveTo(x-5,y-cabT+2+bump);ctx.lineTo(x+9,y-cabT+2+bump);ctx.lineTo(x+15,y-16+bump);ctx.lineTo(x-5,y-16+bump);ctx.closePath();ctx.fill();}
    if(tp==='sport'){ctx.fillStyle=tint(col,-25); // 尾翼
      ctx.fillRect(x-32,y-22+bump,8,3);ctx.fillRect(x-29,y-19+bump,3,5);}
    if(tp==='suv'){ctx.strokeStyle=tint(col,-30);ctx.lineWidth=2; // 車頂架
      ctx.beginPath();ctx.moveTo(x-8,y-cabT-2+bump);ctx.lineTo(x+12,y-cabT-2+bump);ctx.stroke();}
    if(tp==='taxi'){ctx.fillStyle='#ffd23a';rr(x-2,y-cabT-7+bump,14,7,2);ctx.fill(); // 車頂燈
      ctx.fillStyle='#5b4023';ctx.font='bold 6px sans-serif';ctx.fillText('TAXI',x,y-cabT-1.5+bump);}
    ctx.fillStyle='#ffe9a8';ctx.fillRect(x+27,y-11+bump,4,4); // 車燈
    ctx.fillStyle='#2a2a2a';ctx.beginPath();ctx.arc(x-17,y+3,7,0,7);ctx.arc(x+16,y+3,7,0,7);ctx.fill(); // 車輪
    ctx.fillStyle='#cfcfcf';ctx.beginPath();ctx.arc(x-17,y+3,3,0,7);ctx.arc(x+16,y+3,3,0,7);ctx.fill();
    ctx.restore();
    // 乘客（探出天窗）：玩家＋伴侶（小貨車坐前艙）
    const dir=f===1?-1:1, hx=tp==='pickup'?8*dir:-4*dir;
    drawRiderHead(x+hx,y-cabT-3+bump,player.hair);
    if(player.love)drawRiderHead(x+hx-12*dir,y-cabT-2+bump,player.love.ap.hair,wed);
  } else { // 正面(0)/背面(3)
    const back=(f===3);
    ctx.beginPath();ctx.ellipse(x,y+4,20,7,0,0,7);ctx.fill();
    let g=ctx.createLinearGradient(x,y-cabT-1+bump,x,y+4+bump);
    g.addColorStop(0,tint(col,45));g.addColorStop(1,tint(col,-10));
    ctx.fillStyle=g;rr(x-17,y-cabT+1+bump,34,cabT+3,9);ctx.fill();
    ctx.strokeStyle='rgba(0,0,0,.22)';ctx.lineWidth=1.5;rr(x-17,y-cabT+1+bump,34,cabT+3,9);ctx.stroke();
    ctx.fillStyle='#bfe3f2';rr(x-12,back?y-14+bump:y-cabT+3+bump,24,9,4);ctx.fill(); // 擋風玻璃
    if(tp==='pickup'&&back){ctx.fillStyle=tint(col,-30);rr(x-14,y-12+bump,28,10,2);ctx.fill();} // 後斗擋板
    if(tp==='taxi'){ctx.fillStyle='#ffd23a';rr(x-7,y-cabT-5+bump,14,6,2);ctx.fill();}
    if(back){ctx.fillStyle='#d33a2a';ctx.fillRect(x-14,y+bump,6,3);ctx.fillRect(x+8,y+bump,6,3);} // 尾燈
    else{ctx.fillStyle='#ffe9a8';ctx.fillRect(x-14,y+bump,6,4);ctx.fillRect(x+8,y+bump,6,4);} // 頭燈
    ctx.fillStyle='#2a2a2a';rr(x-20,y-8,5,12,2);ctx.fill();rr(x+15,y-8,5,12,2);ctx.fill(); // 側輪
    drawRiderHead(x+(player.love?-7:0),y-cabT-4+bump,player.hair);
    if(player.love)drawRiderHead(x+8,y-cabT-3+bump,player.love.ap.hair,wed);
  }
}
...
function drawPet(p){ drawPetActor(p.kind,p.x,p.y,p.face,p.walk);
  ...
  if(p.say>0){ const cries=p.kind==='dog'?['汪汪！','汪！','嗚汪～']:['喵～','喵嗚～','咕嚕嚕…']; // v52 叫聲泡泡
    drawChatBubble(p.x,p.y+30,cries[p.cry||0]); } }
...
function updateStars(dt){ starT-=dt;
  if(starT<=0){ starT=55+Math.random()*70;
    ...
    const pool=(world==='cn'&&typeof STARS_CN!=='undefined')?STARS_CN:(world==='jp'&&typeof STARS_JP!=='undefined')?STARS_JP:STARS_TW; // v57 日本明星
...
function update(dt){
  ...
  let spd=player.driving?505:player.sailing?420:(run?385:255)*(player.buffSpd>0?1.25:1); // v52 開車505>警車460
  if(typhoon&&typhoon.flood>0.4&&!player.sailing&&!player.riding&&!player.driving)spd*=(1-typhoon.flood*0.32); // v43 淹水涉水變慢
  ...
  for(const c of citizens){ if(c.dead){ c.deadT-=dt; continue; } // 已被擊倒（屍體）不再行動
    ...
    if(c.buddy&&citizens.includes(c.buddy)){
      ...
      if(dd<64&&c.chatCd<=0&&Math.random()<0.03){ c.talk=bd.talk=2.4; c.chatCd=bd.chatCd=8;
        const pool=(world==='cn'&&typeof CN_CHAT_LINES!=='undefined')?CN_CHAT_LINES:(world==='jp'&&typeof JP_CHAT_LINES!=='undefined')?JP_CHAT_LINES:CHAT_LINES; // v41/v57 各地路人口音
  ...
  if(started&&!player.jailed){
    if(!godz){ godzT-=dt;
      if(godzT<=0){ let sp=null;
        ...
        if(sp){ godz={x:sp.x,y:sp.y,hp:GODZ_HP,t:3600,boltT:2,bolt:0,btx:0,bty:0,dir:Math.random()<0.5?1:-1,hit:0}; // v55 停留至少1小時（現實時間）
          sfx('sad');
...
function drawTrainCars(r){ // 長列列車：高鐵=白身飾帶(大陸藍/台灣橘)、普速=綠皮車黃帶；沿軌道轉向
  ...
  const BAND=reg?'#f2d24a':(world==='cn'||world==='jp'?'#2f6fd6':'#f28a2a'); // v57 新幹線同為白身藍帶
...
function drawBoat(x,y,face,moving){ // v55 五船型×五色
  const tp=player.boatType||'fishing', col=player.boatColor||'#c98a4a';
  const W=tp==='carrier'?44:tp==='cruise'?40:tp==='ocean'?32:25; // 船身半寬
  ctx.fillStyle='rgba(0,0,40,.2)';ctx.beginPath();ctx.ellipse(x,y+8,W+3,10,0,0,7);ctx.fill();
  if(moving){ctx.strokeStyle='rgba(255,255,255,.45)';ctx.lineWidth=2;
    const bx=-DIRV[face][0],by=-DIRV[face][1];
    for(let i=1;i<=2;i++){ctx.beginPath();ctx.arc(x+bx*i*(W*0.7),y+6+by*i*12,5+i*4,0,7);ctx.stroke();}}
  let g=ctx.createLinearGradient(x,y-12,x,y+14);
  g.addColorStop(0,tint(col,20));g.addColorStop(1,tint(col,-35));
  ctx.fillStyle=g;
  ctx.beginPath();ctx.moveTo(x-W,y-5);ctx.quadraticCurveTo(x,y+17,x+W,y-5); // 船殼
  ctx.quadraticCurveTo(x+W+2,y-11,x+W-5,y-11);ctx.lineTo(x-W+5,y-11);
  ctx.quadraticCurveTo(x-W-2,y-11,x-W,y-5);ctx.closePath();ctx.fill();
  ctx.strokeStyle=tint(col,-55);ctx.lineWidth=2;ctx.stroke();
  if(tp==='fishing'){ // 小漁船：木甲板＋小旗
  ...
  }else if(tp==='yacht'){ // 遊艇：流線艙房＋大窗
    ctx.fillStyle='#f8f8f4';ctx.beginPath();
    ctx.moveTo(x-14,y-11);ctx.quadraticCurveTo(x-12,y-24,x+2,y-24);ctx.lineTo(x+14,y-24);
    ctx.quadraticCurveTo(x+20,y-18,x+18,y-11);ctx.closePath();ctx.fill();
    ctx.fillStyle='#8fd0e8';for(let i=0;i<3;i++)rr(x-8+i*9,y-21,7,5,2),ctx.fill();
    ctx.strokeStyle='#c8c8c0';ctx.lineWidth=1.5;
    ctx.beginPath();ctx.moveTo(x-14,y-24);ctx.lineTo(x-20,y-30);ctx.stroke(); // 天線
  }else if(tp==='ocean'){ // 遠洋漁船：駕駛樓＋吊桿
    ctx.fillStyle='#f0ece0';rr(x+6,y-27,18,16,3);ctx.fill();
    ctx.fillStyle='#8fd0e8';rr(x+9,y-24,12,6,2);ctx.fill();
    ctx.strokeStyle='#5a4a3a';ctx.lineWidth=2.5;
    ctx.beginPath();ctx.moveTo(x-14,y-11);ctx.lineTo(x-14,y-34);ctx.moveTo(x-14,y-30);ctx.lineTo(x+2,y-16);ctx.stroke(); // 桅杆吊桿
    ctx.strokeStyle='rgba(90,74,58,.6)';ctx.lineWidth=1;
    ctx.beginPath();ctx.moveTo(x+2,y-16);ctx.lineTo(x+2,y-11);ctx.stroke();
  }else if(tp==='cruise'){ // 郵輪：雙層白艙＋圓窗排＋煙囪
    ctx.fillStyle='#fafaf6';rr(x-30,y-22,60,11,3);ctx.fill();
    rr(x-22,y-31,44,10,3);ctx.fill();
    ctx.fillStyle='#8fd0e8';for(let i=0;i<6;i++){ctx.beginPath();ctx.arc(x-24+i*10,y-17,2.2,0,7);ctx.fill();}
    for(let i=0;i<4;i++){ctx.beginPath();ctx.arc(x-15+i*10,y-26,2.2,0,7);ctx.fill();}
    ctx.fillStyle='#e2574c';rr(x+12,y-40,9,10,2);ctx.fill(); // 煙囪
    ctx.fillStyle='#3a3a3a';rr(x+12,y-41,9,3,1);ctx.fill();
  }else if(tp==='carrier'){ // 航空母艦：平甲板＋跑道線＋艦島＋小戰機
    ctx.fillStyle='#6a7480';rr(x-W-2,y-16,W*2+4,7,2);ctx.fill(); // 飛行甲板
    ctx.strokeStyle='#f2d24a';ctx.lineWidth=1.5;ctx.setLineDash([6,4]);
    ctx.beginPath();ctx.moveTo(x-W+2,y-12.5);ctx.lineTo(x+W-2,y-12.5);ctx.stroke();ctx.setLineDash([]);
    ctx.fillStyle='#8a94a0';rr(x+18,y-32,12,16,2);ctx.fill(); // 艦島
    ctx.fillStyle='#8fd0e8';rr(x+20,y-29,8,4,1);ctx.fill();
    ctx.strokeStyle='#5a626c';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x+24,y-32);ctx.lineTo(x+24,y-38);ctx.stroke(); // 天線
    ctx.fillStyle='#d8dde2';ctx.beginPath(); // 甲板小戰機
    ctx.moveTo(x-18,y-17);ctx.lineTo(x-8,y-20);ctx.lineTo(x-18,y-23);ctx.closePath();ctx.fill();
  }
}
...
function draw(){
  ...
  if(RAILNET)for(const e of RAILNET.edges){ const P=e.pts;
    for(let i=0;i<P.length-1;i++){
      ...
      if(e.ferry){ // 火車輪渡航線：海上藍虛線
        ctx.setLineDash([16,12]);ctx.strokeStyle='rgba(60,105,150,.6)';ctx.lineWidth=4;
        ctx.beginPath();ctx.moveTo(ax,ay);ctx.lineTo(bx,by);ctx.stroke();
        ctx.setLineDash([]); continue; }
      if(e.tunnel){ // v57 海底隧道（青函）：灰虛線
        ctx.setLineDash([10,8]);ctx.strokeStyle='rgba(90,90,100,.55)';ctx.lineWidth=5;
        ctx.beginPath();ctx.moveTo(ax,ay);ctx.lineTo(bx,by);ctx.stroke();
        ctx.setLineDash([]); continue; }
  ...
  list.push({y:player.y,f:()=>{
    ...
    if(player.ferris){ // 摩天輪車廂裡露出頭
      ...
      return;}
    if(player.driving){ drawCarRide(); } // v52 開車：小紅車＋全員上車
    else if(player.sailing){
  ...
  // v41 大陸北方寒冷飄雪（越往北越冷、雪越大）；v57 日本北海道/東北同樣飄雪
  if(world==='cn'||world==='jp'){ const cold=Math.max(0,Math.min(1,(250-player.y/TILE)/180));
...
function drawUI(){
  ...
  if(world==='jp'){ dispMin=gameMin+60; } // v57 日本時區 UTC+9（比台灣快1小時）
  const gh=Math.floor((dispMin+2880)/60)%24, gm=Math.floor(((gameMin%60)+60)%60);
  ctx.fillText(`${String(gh).padStart(2,'0')}:${String(gm).padStart(2,'0')}`+(world==='cn'?(tzoff>0?' 西'+tzoff+'區':tzoff<0?' 東'+(-tzoff)+'區':' 京'):world==='jp'?' JST':''),30,68);
  ...
  if(ui==='map'){
    ...
    // v55 標註機場✈️與港口⚓（依當前世界的實際建築位置）
    ctx.font='13px serif';
    for(const b2 of BUILDINGS){ if(b2.t!=='airport'&&b2.t!=='harbor')continue;
      const px2=x+(b2.tx+b2.tw/2)/MW*mw3, py2=y+(b2.ty+b2.th/2)/MH*mh;
      ctx.fillText(b2.t==='airport'?'✈️':'⚓',px2-7,py2+5);}
    if(RAILNET)for(const e of RAILNET.edges)if(e.ferry){ // 鐵路輪渡兩端＝港口（粵海/沖繩航路）
      for(const p2 of [e.pts[0],e.pts[e.pts.length-1]]){
        const px2=x+p2[0]/MW*mw3, py2=y+p2[1]/MH*mh; ctx.fillText('⚓',px2-7,py2+5);} }
...
function save(){ try{ localStorage.setItem(SAVEKEY,JSON.stringify({
  ...
  outfit:player.outfit,deco:player.deco,tie:player.tie,ownClothes:player.ownClothes,money,inv,dex,boat:player.boat,carRent:player.carRent?1:0,
  carType:player.carType,carColor:player.carColor,boatType:player.boatType,boatColor:player.boatColor, // v55 車船型色
  ...
  job:player.job,farms:myFarms,world,pets:player.pets,petFood:player.petFood,ownPetAccs:player.ownPetAccs,autographs:player.autographs,
...
function load(){ try{ const s=JSON.parse(localStorage.getItem(SAVEKEY));
  ...
  player.pets=Array.isArray(s.pets)?s.pets:(s.pet?[s.pet]:[]); // v41/v49 寵物（向下相容舊單一 pet 存檔）
  player.pets.forEach((p,i)=>{ if(p.slot==null)p.slot=i; if(p.acc===undefined)p.acc=null; });
  player.petFood=s.petFood||0; player.ownPetAccs=s.ownPetAccs||[];
  ...
  player.boat=!!s.boat;player.carRent=!!s.carRent;player.driving=false;musicOn=s.music!==false; // v52 車輛持有跨存檔、載入時下車
  player.carType=s.carType||'sedan';player.carColor=s.carColor||'#c93a2e'; // v55
  player.boatType=s.boatType||'fishing';player.boatColor=s.boatColor||'#c98a4a';
  setScene(['cn','jp'].includes(s.world)?s.world:'tw'); genWorld(); genNpcs(); // v40/v57 切到存檔所在世界，重生該世界物件/NPC
