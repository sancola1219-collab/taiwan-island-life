...
/* ================= 圖塊預繪 ================= */
const tileImgs={};
const TPRI={[SEA]:0,[LAKE]:0,[SAND]:1,[FIELD]:2,[GRASS]:2,[HIGH]:2,[MT]:2,[PLAZA]:3,[PATH]:3};
const TCOL={[SEA]:'#3f8fbd',[LAKE]:'#4694b4',[SAND]:'#efe0a8',[GRASS]:'#6fb257',[HIGH]:'#559549',
  [FIELD]:'#aacb6c',[PLAZA]:'#d8cba8',[PATH]:'#d9b97e',[MT]:'#8b8f7c'};
const CORNERS=[[-1,-1,0],[1,-1,Math.PI/2],[1,1,Math.PI],[-1,1,Math.PI*1.5]];
function makeTile(type,sv){
  ...
  const organic=(base,dark,light,s)=>{ // 參考圖的有機草地紋理（不規則色斑）
    g.fillStyle=base;g.fillRect(0,0,TILE,TILE);
    for(let i=0;i<9;i++){g.fillStyle=dark;
      g.beginPath();g.ellipse(hsh(i+s,3)*48,hsh(i+s,7)*48,3+hsh(i+s,11)*5,2+hsh(i+s,13)*4,hsh(i,s)*3,0,7);g.fill();}
    for(let i=0;i<6;i++){g.fillStyle=light;
      g.beginPath();g.ellipse(hsh(i+s,17)*48,hsh(i+s,23)*48,2+hsh(i+s,29)*4,2+hsh(i+s,31)*3,0,0,7);g.fill();}
    g.strokeStyle=dark;g.lineWidth=1.5;
    for(let i=0;i<5;i++){const vx=hsh(i+s,37)*44+2,vy=hsh(i+s,41)*44+2;
      g.beginPath();g.moveTo(vx,vy+3);g.lineTo(vx+1.5,vy);g.moveTo(vx+3,vy+3);g.lineTo(vx+4,vy);g.stroke();}};
  if(type===GRASS)organic('#6fb257','#63a24b','#7abb60',1+s0);
  else if(type===HIGH)organic('#559549','#4b8a40','#61a353',2+s0);
...
tileImgs[GRASS+100]=makeTile(GRASS,5);tileImgs[HIGH+100]=makeTile(HIGH,5);
// 小地圖
...
const BUILDING_DRAWS={
 ...
 archbridge(b){ // 紅欄白拱跨海橋（依台東縣參考圖重繪）
  const x=b.x-24,y=b.y+14,w=b.w+48;
  // 水中倒影
  ctx.strokeStyle='rgba(20,40,80,.22)';ctx.lineWidth=5;
  for(let i=0;i<4;i++){const cxA=x+w*(i+0.5)/4;
    ctx.beginPath();ctx.arc(cxA,y+18,w/8-7,0.2,Math.PI-0.2);ctx.stroke();}
  // 白色拱圈（水上）
  ctx.strokeStyle='#f4f4f0';ctx.lineWidth=6;
  for(let i=0;i<4;i++){const cxA=x+w*(i+0.5)/4;
    ctx.beginPath();ctx.arc(cxA,y+8,w/8-7,Math.PI,0);ctx.stroke();}
  // 弧形橋面
  ctx.strokeStyle='#e8e8e2';ctx.lineWidth=8;
  ctx.beginPath();ctx.moveTo(x,y);ctx.quadraticCurveTo(x+w/2,y-26,x+w,y);ctx.stroke();
  ctx.strokeStyle='rgba(0,0,0,.12)';ctx.lineWidth=2;
  ctx.beginPath();ctx.moveTo(x,y+4);ctx.quadraticCurveTo(x+w/2,y-22,x+w,y+4);ctx.stroke();
  // 紅色欄杆與立柱
  ctx.strokeStyle='#d84f43';ctx.lineWidth=3;
  ctx.beginPath();ctx.moveTo(x,y-8);ctx.quadraticCurveTo(x+w/2,y-34,x+w,y-8);ctx.stroke();
  for(let i=0;i<=8;i++){const t2=i/8, bx2=x+w*t2, by2=y-(1-(2*t2-1)*(2*t2-1))*13;
    ctx.beginPath();ctx.moveTo(bx2,by2-1);ctx.lineTo(bx2,by2-9);ctx.stroke();}
  // 兩端燈柱
  for(const ex of [x-5,x+w+5]){
    ctx.fillStyle='#5a4a3a';ctx.fillRect(ex-2,y-26,4,28);
    ctx.fillStyle=isNight()?'#ffd97a':'#e8e0c8';ctx.beginPath();ctx.arc(ex,y-30,5,0,7);ctx.fill();}
  drawRoofSign(x+w/2,y-48,b.label,'#c9503f');},
 ...
 myhome(b){const x=b.x,y=b.y,w=b.w,h=b.h;bShadow(x,y+h,w);
  ...
  ctx.fillStyle=isNight()?'#ffe9a0':'#cfe3f5';rr(x+8,y-16,20,16,3);ctx.fill();rr(x+w-28,y-16,20,16,3);ctx.fill();
  ctx.fillStyle='#e2574c';ctx.font='11px serif';ctx.fillText('🌼',x+4,y+h-4);ctx.fillText('🌼',x+w-16,y+h-4);
  drawRoofSign(x+w/2,y-64,'🏠 '+b.label,'#c9705f');},
...
function genWorld(){
  ...
  for(let ty=2;ty<MH-2;ty++)for(let tx=2;tx<MW-2;tx++){
    ...
    if(cluster>0.66&&rand()<0.11&&trees.length<800&&!nearBuilding(tx,ty)){
      if(t===HIGH){ const r2=rand();
        if(r2<0.25)trees.push({x:(tx+0.5)*TILE,y:(ty+0.8)*TILE,kind:'fruit',fruit:'水蜜桃',has:true,regrow:0,shake:0});
        else if(r2<0.42)trees.push({x:(tx+0.5)*TILE,y:(ty+0.8)*TILE,kind:'fruit',fruit:null,pink:true,has:false,regrow:0,shake:0});
        else trees.push({x:(tx+0.5)*TILE,y:(ty+0.8)*TILE,kind:'pine',fruit:null,has:true,regrow:0,shake:0});}
      else{ const f=fruitOf(tx,ty);
        if(f){trees.push({x:(tx+0.5)*TILE,y:(ty+0.8)*TILE,kind:'fruit',fruit:f,has:true,regrow:0,shake:0});
          if(rand()<0.5&&trees.length<800) // 成叢種植（參考圖的樹叢感）
            trees.push({x:(tx+(rand()<0.5?1.4:-0.4))*TILE,y:(ty+1.4)*TILE,kind:'fruit',fruit:f,has:true,regrow:0,shake:0});}}
      continue;}
...
const player={x:spawn.x,y:spawn.y,face:0,walk:0,moving:false,tool:0,
  ...
  boat:false,sailing:false,hp:100,hunger:100,race:0,soak:null,tired:0};
...
function talkTo(npc){ if(!npc)return;
  ...
  if(st.s<3){ const t=chain[st.s], have=inv[t[0]]||0, it=ITEMS[t[0]];
    const opts=[];
    if(have>=t[1])opts.push({label:'交付 '+it.e+t[0]+'×'+t[1]+'（獎勵 '+fmt(t[2])+'元）',cb(){
      inv[t[0]]-=t[1]; if(inv[t[0]]<=0)delete inv[t[0]];
      money+=t[2]; st.s++; sfx('jingle'); save();
      if(st.s>=3)dlg(npc.name,['三個委託都完成了，太感謝你！','我們現在是最好的夥伴——','隨時來找我「結伴同行」，一起環島吧！🎉']);
...
function buildAct(b){
  ...
  const L={t101(){ openMenu('台北101：全台最高樓！',[
   ...
   market(){ openMenu('🏮 '+b.label+'：想吃點什麼？',[
     ...
     {label:'離開',cb(){ui=null;}}]);},
   temple(){ openMenu(b.label+'：要參拜嗎？',[
     {label:'參拜（香油錢 100元）🙏',cb(){ if(money>=100){money-=100;sfx('cash');save();ui=null;
   ...
   eatery(){ const t=TASK_POOL[Math.abs(Math.floor(Math.sin(b.tx*13.7+b.ty*7.3)*1e4))%TASK_POOL.length];
     ...
     else opts.push({label:'老闆委託：'+ITEMS[t[0]].e+t[0]+'×'+t[1]+'（酬勞'+fmt(t[2])+'元）',cb(){
...
function collectStamp(label){ if(!label||stamps[label])return;
  ...
  sfx('chime');save();}
let myHome=null, eateryDone={}, groundToys=[], projs=[];
...
function buildHome(){
  if(myHome){dlg('蓋房子',['你已經有家囉！','就在 '+(regionOf())+' 附近，回去看看吧。']);return;}
  const wood=inv['木材']||0, ore=inv['礦石']||0;
  if(wood<10||ore<5||money<2000){
    dlg('蓋房子',['蓋自己的家需要：','木材×10（現有'+wood+'）＋礦石×5（現有'+ore+'）＋2000元','努力收集吧！']);return;}
  const ftx=Math.floor(player.x/TILE)+DIRV[player.face][0]*3-2,
        fty=Math.floor(player.y/TILE)+DIRV[player.face][1]*3-2;
  let ok=true;
  for(let yy=0;yy<3&&ok;yy++)for(let xx=0;xx<4&&ok;xx++){const t=T(ftx+xx,fty+yy);
    if(!(t===GRASS||t===FIELD||t===SAND||t===HIGH||t===PATH))ok=false;}
  if(ok)for(const b of BUILDINGS)if(ftx<b.tx+b.tw+1&&ftx+5>b.tx&&fty<b.ty+b.th+2&&fty+5>b.ty){ok=false;break;}
  if(!ok){dlg('蓋房子',['這裡不夠平坦寬敞…','找一塊空曠的草地，面向它再蓋一次！']);return;}
  inv['木材']-=10;if(inv['木材']<=0)delete inv['木材'];
  inv['礦石']-=5;if(inv['礦石']<=0)delete inv['礦石'];
  money-=2000; myHome={tx:ftx,ty:fty};
  addBuild('myhome',ftx,fty,4,3,player.name+'的家');
  sfx('jingle'); save(); ui=null;
  toast('🏠 蓋好自己的家了！走到門口就能進去睡覺');
}
function playToy(n){
...
function openCraft(){
  ...
  openMenu('🔨 製作（木材×'+wood+'　礦石×'+ore+'）',[
    {label:'🏠 蓋自己的家（木材×10＋礦石×5＋2000元）',cb(){buildHome();}},
...
function drawTree(tr){
  ...
  if(tr.kind==='palm'){
    ...
    const pk=tr.pink;
    let g=ctx.createRadialGradient(tr.x-8+sh,tr.y-52,4,tr.x+sh,tr.y-40,32);
    if(pk){g.addColorStop(0,'#f8cfe0');g.addColorStop(0.6,'#f2aac8');g.addColorStop(1,'#d886ab');}
    else{g.addColorStop(0,'#66ab52');g.addColorStop(0.6,'#4a9142');g.addColorStop(1,'#3a7836');}
    ctx.fillStyle=g;
    ctx.beginPath();ctx.arc(tr.x-14+sh,tr.y-33,16,0,7);ctx.arc(tr.x+14+sh,tr.y-33,16,0,7);
    ctx.arc(tr.x+sh,tr.y-47,19,0,7);ctx.arc(tr.x+sh,tr.y-28,16,0,7);ctx.fill();
    ctx.fillStyle=pk?'rgba(150,80,110,.22)':'rgba(25,60,25,.22)'; // 底部陰影
    ctx.beginPath();ctx.arc(tr.x+sh,tr.y-26,15,0,Math.PI);ctx.fill();
    ctx.fillStyle=pk?'rgba(255,240,248,.55)':'rgba(200,240,170,.4)'; // 頂部受光
    ctx.beginPath();ctx.arc(tr.x-7+sh,tr.y-49,8,0,7);ctx.fill();
...
/* ================= 地標貼圖（玩家參考圖裁切，直接當遊戲圖用） ================= */
const LM_SPRITES={
 '台北101':{f:'tpe101',w:2.2},'九份茶樓':{f:'jiufen',w:2.4},'野柳女王頭':{f:'yehliu',w:2.6},
 '鶯歌陶瓷老街':{f:'yingge',w:1.9},'三峽老街':{f:'sanxia',w:1.9},'淡水漁人碼頭':{f:'tamsui',w:2.3},
 '基隆鼎邊銼':{f:'miaokou',w:2.3},'基隆港':{f:'keelungport',w:2.7},
 '大溪老街':{f:'daxi',w:1.9},'拉拉山神木':{f:'lalashan',w:2.4},
 '六福村':{f:'leofoo',w:2.4},'內灣老街':{f:'neiwan',w:1.9},'北埔老街':{f:'beipu',w:1.9},
 '高美濕地':{f:'gaomei',w:2.3},'大甲鎮瀾宮':{f:'dajia',w:1.8},'台中歌劇院':{f:'opera',w:2.0},
 '宮原眼科':{f:'gongyuan',w:1.9},'彩虹眷村':{f:'rainbow',w:2.1},
 '鹿港天后宮':{f:'lukang',w:1.8},'八卦山大佛':{f:'buddha',w:2.3},'梧棲港':{f:'wanggong',w:2.5},
 '日月潭碼頭':{f:'sunmoon',w:2.9},'九族文化村':{f:'nine',w:2.3},'溪頭妖怪村':{f:'xitou',w:2.1},
 '合歡山':{f:'hehuan',w:3.2},'北港朝天宮':{f:'beigang',w:1.8},'西螺老街':{f:'douliu',w:2.1},
 '奮起湖老街':{f:'fenqihu',w:2.1},'故宮南院':{f:'southpalace',w:2.1},'高跟鞋教堂':{f:'highheel',w:2.4},
 '東石漁人碼頭':{f:'dongshi',w:2.5},'檜意森活村':{f:'hinoki',w:2.1},'嘉義火雞肉飯':{f:'wenhua',w:2.4},
 '赤崁樓':{f:'chikan',w:1.9},'安平古堡':{f:'anping',w:2.1},'花園夜市':{f:'garden',w:2.7},
 '高雄港':{f:'pier2',w:2.5},'龍虎塔':{f:'dragon',w:2.3},'高雄85大樓':{f:'t85',w:2.7},
 '旗津老街':{f:'cijin',w:2.1},'恆春古城':{f:'hengchun',w:2.3},'屏東海生館':{f:'aquarium',w:2.1},
 '小琉球碼頭':{f:'liuqiu',w:2.5},'礁溪溫泉':{f:'jiaoxi',w:2.3},'宜蘭傳藝中心':{f:'chuanyi',w:2.1},
 '蘭陽博物館':{f:'lanyang',w:2.1},'太平山檜木':{f:'taiping',w:2.3},
 '太魯閣':{f:'taroko',w:2.5},'清水斷崖':{f:'qingshui',w:2.7},'七星潭':{f:'qixingtan',w:2.7},
 '三仙台八拱橋':{f:'sanxiantai',w:3.4},'鹿野高台':{f:'balloonhi',w:2.3},'知本溫泉':{f:'zhiben',w:2.3},
 '澎湖天后宮':{f:'penghumatsu',w:1.9},'澎湖跨海大橋':{f:'xbridge',w:3.0},'雙心石滬':{f:'heartweir',w:2.7},
 '吉貝沙尾':{f:'jibei',w:2.7},'莒光樓':{f:'juguang',w:2.1},'山后聚落':{f:'shanhou',w:2.3},
 '芹壁石頭屋':{f:'qinbi',w:2.3}};
STATIONS.forEach(s=>LM_SPRITES[s.n]={f:'shengxing',w:1.55}); // 全部車站用勝興車站圖
const SPRC={};
function sprImg(f){ if(!SPRC[f]){const im=new Image();im.src='sprites/'+f+'.png';SPRC[f]=im;} return SPRC[f];}
function drawLmSprite(b,im,mult){
  const W=b.w*(mult||1.8), H=W*im.naturalHeight/im.naturalWidth;
  const cx3=b.x+b.w/2, by3=b.y+b.h+10;
  ctx.fillStyle='rgba(0,0,0,.18)';
  ctx.beginPath();ctx.ellipse(cx3,by3-4,W*0.42,12,0,0,7);ctx.fill();
  ctx.save();rr(cx3-W/2,by3-H,W,H,16);ctx.clip();
  ctx.drawImage(im,cx3-W/2,by3-H,W,H);ctx.restore();
  ctx.strokeStyle='rgba(90,64,35,.30)';ctx.lineWidth=2.5;
  rr(cx3-W/2,by3-H,W,H,16);ctx.stroke();
}
/* ================= 繪製主流程 ================= */
let tGlobal=0;
function draw(){
  ...
  for(let ty=y0;ty<=y1;ty++)for(let tx=x0;tx<=x1;tx++){
    const t=T(tx,ty);
    let img=tileImgs[t];
    if((t===GRASS||t===HIGH)&&hsh(tx*3.7,ty*5.1)>0.5)img=tileImgs[t+100];
    ctx.drawImage(img,tx*TILE,ty*TILE);
    // 淺水帶（岸邊變淺色）
    if((t===SEA||t===LAKE)&&(WALKABLE[T(tx,ty-1)]||WALKABLE[T(tx,ty+1)]||WALKABLE[T(tx-1,ty)]||WALKABLE[T(tx+1,ty)])){
      ctx.fillStyle='rgba(135,208,230,.45)';ctx.fillRect(tx*TILE,ty*TILE,TILE,TILE);}
    // 圓角地形過渡（柔化海岸與路緣）
    const pt2=TPRI[t];
    for(const [cdx,cdy,a0] of CORNERS){
      const n1=T(tx+cdx,ty),n2=T(tx,ty+cdy);
      if(n1===n2&&TPRI[n1]>pt2){
        const px4=tx*TILE+(cdx>0?TILE:0), py4=ty*TILE+(cdy>0?TILE:0);
        ctx.fillStyle=TCOL[n1];
        ctx.beginPath();ctx.moveTo(px4,py4);ctx.arc(px4,py4,TILE*0.55,a0,a0+Math.PI/2);
        ctx.closePath();ctx.fill();}}
    // 田埂
    if(t===FIELD){ctx.strokeStyle='#8aa04b';ctx.lineWidth=3;
      if(T(tx,ty-1)!==FIELD){ctx.beginPath();ctx.moveTo(tx*TILE,ty*TILE+1);ctx.lineTo(tx*TILE+TILE,ty*TILE+1);ctx.stroke();}
      if(T(tx,ty+1)!==FIELD){ctx.beginPath();ctx.moveTo(tx*TILE,ty*TILE+TILE-1);ctx.lineTo(tx*TILE+TILE,ty*TILE+TILE-1);ctx.stroke();}
      if(T(tx-1,ty)!==FIELD){ctx.beginPath();ctx.moveTo(tx*TILE+1,ty*TILE);ctx.lineTo(tx*TILE+1,ty*TILE+TILE);ctx.stroke();}
      if(T(tx+1,ty)!==FIELD){ctx.beginPath();ctx.moveTo(tx*TILE+TILE-1,ty*TILE);ctx.lineTo(tx*TILE+TILE-1,ty*TILE+TILE);ctx.stroke();}}
    if(t===SEA||t===LAKE){
  ...
  if(ui==='bag'){
    ...
    names.slice(0,cols*rows).forEach((n,i)=>{
      ...
      uiHits.push({x:gx,y:gy,w:94,h:94,cb:((nm)=>()=>{
        if(ITEMS[nm].toy)playToy(nm); else eatFood(nm);})(n)});});
...
function save(){ try{ localStorage.setItem(SAVEKEY,JSON.stringify({
  ...
  gameDay,gameMin:Math.floor(gameMin),tired:Math.floor(player.tired),myHome,eateryDone,
...
function load(){ try{ const s=JSON.parse(localStorage.getItem(SAVEKEY));
  ...
  eateryDone=s.eateryDone||{};myHome=s.myHome||null;
  if(myHome)addBuild('myhome',myHome.tx,myHome.ty,4,3,(s.name||'小島民')+'的家');
